...
function buySelectedItems() {
    ...
    let queryString = itemsToBuy.map(item => `name=${encodeURIComponent(item.name)}&price=${item.price}&quantity=${item.quantity}`).join('&');
    const newUrl = `https://www.bing.com?${queryString}`;
